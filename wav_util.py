...
class wav_writer:
    ...
    def __init__(
        self, output_path: str, channels: int, sampling_freq: int, bits_per_sample: int
    ):
        # generate wav header
        ...
        # redundant info
        self.wav_header.BytePerBloc = int(
            (self.wav_header.BitsPerSample / 8) * self.wav_header.NbrChannels
        )
        self.wav_header.BytePerSec = int(
            self.wav_header.BytePerBloc * self.wav_header.Frequency
        )
